...
def doWork(train, user):
    ...
    model.save_weights('D:\zs17\model\\user_{}_checkpoint_{}_epoch_{}.hdf5'.format(i, HIDDEN_DIM, nb_epoch))
...
train=pd.read_csv("D:\zs17\\data.csv")
train['event']= train['event'].astype("category")
userdata=pd.read_csv("D:\zs17\\user.csv")
users = userdata['PID']
users=users[100:] #change here a. user[:100] b. user[2500:]
not_done=[]
...
for user in users:
    ...
    try:
        ...
        submission[user]=out
        file='D:\zs17\output\\{}.json'.format(user)
        ...
        continue
    if(x==50):
        f1='D:\zs17\\notdone{}.csv'.format(user)
        f2='D:\zs17\\submission{}.csv'.format(user)
...
remaining = pd.DataFrame()
remaining['notdone'] = not_done
remaining.to_csv('D:\zs17\\notdone.csv')
submission.to_csv('D:\zs17\\submission.csv')
tf = timer()
